...
export default function TrailDetailPanel({ trail }: Props) {
  ...
  const setTrailheadCoords = useTrailStore((s) => s.setTrailheadCoords);
  const setParkingNotes = useTrailStore((s) => s.setParkingNotes);
  ...
  // Save trailhead coords to store whenever geometry is available
  useEffect(() => {
    if (elevCoords && elevCoords.length > 0) {
      const [lon, lat] = elevCoords[0];
      setTrailheadCoords({ lat, lon });
    }
  }, [elevCoords]);
